...
public class DyUrlPluginExecutor extends AbstractPluginExecutor {
  ...
  private final String USER_AGENT = "Dalvik/2.1.0 (Linux; U; Android 10; COL-AL10 Build/HUAWEICOL-AL10)";
  ...
  public PluginResult messageHandler(PluginParam pluginParam) {
    ...
    if (ExecuteTriggerEnum.KEYWORD.equals(pluginParam.getExecuteTriggerEnum())) {
      ...
      pluginResult = getInfo(message, pluginResult);
    }
    return pluginResult;
  }

  public PluginResult getInfo(String info, PluginResult pluginResult) {
    ...
    if (url != null) {
      ...
      if (id != null) {
        ...
        String rel_url = ReUtil
            .findAll("play_addr\":\\{\"uri\":\"(.*?)\",\"url_list\":\\[\"(.*?)\"", json, 2).get(0);
        if (rel_url != null) {
          String video_url = rel_url.replaceAll("playwm", "play");
          HttpResponse response = HttpRequest.get(video_url)
              .header(HttpHeaders.USER_AGENT, USER_AGENT).execute();
          if (HttpStatus.HTTP_MOVED_TEMP == response.getStatus()) {
            pluginResult.setMessage(
                StrUtil.format("视频下载地址为：【{}】，请复制到浏览器进行下载", response.header(HttpHeaders.LOCATION)));
          }
        } else {
          pluginResult.setMessage("获取真正的视频链接失败！");
        }
      } else {
        pluginResult.setMessage("获取视频id失败！");
      }
    } else {
      pluginResult.setMessage("提取视频链接失败！");
    }
    return pluginResult;
  }
}
